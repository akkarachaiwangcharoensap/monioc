...
export interface UseSpreadsheetKeysOptions {
	...
	/** Ref that mirrors the `openCategoryRow` state — read at event time to detect if a dropdown is open. */
	openCategoryRowRef: MutableRefObject<number | null>;
}
...
export function useSpreadsheetKeys(opts: UseSpreadsheetKeysOptions): UseSpreadsheetKeysResult {
	...
	const handleContainerKeyDown = useCallback(
		(e: React.KeyboardEvent) => {
			...
			// Allow events from react-select's hidden input (inside [data-cell-col="1"]) when
			// the dropdown is closed — so arrow keys / Enter navigate cells normally.
			// For all other INPUT elements (name/price cell inputs), bail out as before.
			const isInCategoryCell = target.tagName === 'INPUT' && !!target.closest('[data-cell-col="1"]');
			if (target.tagName === 'INPUT' && (!isInCategoryCell || openCategoryRowRef.current !== null)) return;
			...
			if (f) {
				...
				// Enter/F2 on a selected category cell: open the dropdown.
				// Calling navigate() here would clear the selection and do nothing visible.
				if (f.col === COL_CATEGORY && (e.key === 'Enter' || e.key === 'F2') && useReactSelect) {
					e.preventDefault();
					setOpenCategoryRow(f.row);
					setCategorySearchInput('');
					const id = rowsRef.current[f.row]?._id;
					if (id) setTimeout(() => categorySelectRefs.current.get(id)?.focus(), 0);
					return;
				}
		...
		[undo, redo, selRef, rowsRef, setRows, publishWithHistory, publishTyping, moveSelection, navigate, clearSelection, setSelFocus, setEditingCell, useReactSelect, seedPriceDraft, categorySelectRefs, setOpenCategoryRow, setCategorySearchInput, inputRefs, openCategoryRowRef],
	);

...
export default function SpendingChart({
	...
	// don't replay the entry animation. Re-enable when the underlying data changes.
	const [animationActive, setAnimationActive] = useState(true);
	const normalizedItems = items.filter((item) => Number.isFinite(item.amount) && item.amount > 0);

	// Derive a stable key for the current data set; reset animation when it changes.

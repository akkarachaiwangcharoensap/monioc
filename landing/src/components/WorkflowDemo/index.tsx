...
import { useState, useEffect, useRef, useSyncExternalStore, type ReactElement } from 'react';
...
/* ── prefers-reduced-motion store (SSR-safe via useSyncExternalStore) ─── */
...
function getMotionServerSnapshot(): boolean {
    return false; // safe SSR default; client re-renders after hydration if different
}
...
export default function WorkflowDemo({ className }: WorkflowDemoProps): ReactElement {
    ...
    // Both derived purely from state — no synchronous setState needed.
    const isTransitioning = step !== displayedStep;
    const slideDir: 'left' | 'right' = step > displayedStep ? 'left' : 'right';
    ...
    /* ── prefers-reduced-motion (SSR: false; client: real value after hydration) ── */
    const motionSafe = useSyncExternalStore(subscribeMotion, getMotionSnapshot, getMotionServerSnapshot);

    /* ── Transition classes (respects prefers-reduced-motion) ─────────── */
    const mockupTransitionStyle: React.CSSProperties = isTransitioning
        ? {
            opacity: 0,
    ...
    return (
        <section
            id="workflow"
            ref={revealRef}
            className={`reveal-block stagger border-y border-slate-100 bg-[#F8F8F6] py-16 sm:py-20 ${className ?? ''}`}
        >
...
function StepSidebar({
    ...
    return (
        <>
            ...
            <div className="lg:hidden flex items-center justify-center gap-1.5 overflow-x-auto pb-2 -mx-1 px-1 scrollbar-none" role="tablist" aria-label="Workflow steps">
                {WORKFLOW_STEPS.map((s) => {
                    const isActive = s.id === step;
                    return (
                        <button
